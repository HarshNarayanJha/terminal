...
void TextBuffer::Insert(til::CoordType row, const TextAttribute& attributes, RowWriteState& state)
{
    ...
    const auto restoreAttr = scratchAttr.slice(gsl::narrow<uint16_t>(restoreState.columnBegin), gsl::narrow<uint16_t>(restoreState.columnEnd));
    r.Attributes().replace(gsl::narrow<uint16_t>(state.columnEnd), scratch.size(), restoreAttr);
    r.Attributes().resize_trailing_extent(r.size());
