...
def main():
    ...
    initial_value = build_trie_value(ClusterBreak.ANY, CharacterWidth.Narrow)
    error_value = build_trie_value(ClusterBreak.ANY, CharacterWidth.Ambiguous)
    ...
    for group in root.findall("./ns:repertoire/ns:group", ns):
        ...
        for char in group:
            ...
            if char_extpict == "Y":
                # Currently every single Extended_Pictographic codepoint happens to have the default
                # XX Grapheme Cluster Break property. This is fantastic for us because it means we
                # can stuff it into the ClusterBreak enum and make our algorithm way simpler.
                assert char_gcb == "XX"
